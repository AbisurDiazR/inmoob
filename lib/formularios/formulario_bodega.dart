...
class AgregarBodega extends StatefulWidget {
  @override
  _AgregarBodegaState createState() => _AgregarBodegaState();
}

class _AgregarBodegaState extends State<AgregarBodega> {
  ...
  _agregarBodega() async {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      StorageReference ref = FirebaseStorage.instance.ref().child(images[i].name);
    ...
    database.reference().child('inmuebles').push().set({
      ...
      'fotos': _urls,
      'tipo':'Bodega'
    }).whenComplete(_tareaCompletada);}else{
